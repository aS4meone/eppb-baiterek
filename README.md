...
- **Предприниматель**: кнопка «Войти через eGov» — имитация eGov IDP, вход в один клик.
- **Администратор** (`/admin`): пароль `baiterek2026` (env `EPPB_ADMIN_PASSWORD`).
  Все write-API (`/api/admin/*`, BPM, подача заявок) защищены сессиями; заявки видны
  только владельцу; схема услуги валидируется (zod) перед записью — битая схема
  не может сломать опубликованную услугу.
...
2. Карточка «Приобретение вагонов в лизинг» → «Подать заявку» → вход через eGov (один клик).
